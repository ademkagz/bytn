...
// The zBYTN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBYTN is a hash of the serial
...
{
    //Create an output returning the zBYTN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBYTNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBYTN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBYTN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBYTNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBYTN output", __func__);
...
{
    CzBYTNTracker* zbytnTracker = pwallet->zbytnTracker.get();
    CMintMeta meta;
    if (!zbytnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbytnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BYTN stake is the outpoint
