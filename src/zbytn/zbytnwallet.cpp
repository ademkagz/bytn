...
CzBYTNWallet::CzBYTNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbytn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBYTNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBYTNSeed_deprecated()) {
                    LogPrintf("%s: Updated zBYTN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbytn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbytn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBYTNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBYTNCount(nCountLastUsed);
    else if (!walletdb.ReadZBYTNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBYTNWallet::Lock()
{
    seedMaster = 0;
}

void CzBYTNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBYTNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBYTN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBYTNWallet::LoadMintPoolFromDB()
{
...
void CzBYTNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBYTNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBYTNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbytnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBYTNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBYTN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbytnTracker which also adds to database
    pwalletMain->zbytnTracker->Add(dMint, true);
...
void CzBYTNWallet::SeedToZBYTN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBYTNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBYTNWallet::GenerateDeterministicZBYTN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBYTNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBYTNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBYTNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
