...
CzBYTNTracker::CzBYTNTracker(std::string strWalletFile)
{
...
CzBYTNTracker::~CzBYTNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBYTNTracker::Init()
{
...
bool CzBYTNTracker::Archive(CMintMeta& meta)
{
...
bool CzBYTNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBYTNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBYTNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBYTNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBYTNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBYTNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBYTNTracker::GetSerialHashes()
{
...
CAmount CzBYTNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBYTNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBYTNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBYTNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBYTNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBYTNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBYTNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBYTNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBYTNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBYTNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBYTNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBYTNWallet* zBYTNWallet)
{
    bool iszBYTNWalletInitialized = (NULL != zBYTNWallet);
    CMintMeta meta;
    ...
    if (! iszBYTNWalletInitialized)
        zBYTNWallet = new CzBYTNWallet(strWalletFile);
    meta.isSeedCorrect = zBYTNWallet->CheckSeed(dMint);
    if (! iszBYTNWalletInitialized)
        delete zBYTNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBYTNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBYTNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBYTNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBYTNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBYTNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBYTNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBYTNWallet* zBYTNWallet = new CzBYTNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBYTNWallet);
        }
        delete zBYTNWallet;
        LogPrint("zero", "%s: added %d dzbytn from DB\n", __func__, listDeterministicDB.size());
    }
