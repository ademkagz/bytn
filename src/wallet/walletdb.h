...
public:
    ...
    bool WriteZBYTNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBYTNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBYTNSeed_deprecated(uint256& seed);
    bool EraseZBYTNSeed();
    bool EraseZBYTNSeed_deprecated();

    bool WriteZBYTNCount(const uint32_t& nCount);
    bool ReadZBYTNCount(uint32_t& nCount);
