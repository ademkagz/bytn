...
#include "ui_zbytncontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbytnTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbytnTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBYTN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBYTN seed used to mint this zBYTN is not the same as currently hold in the wallet";
            else
