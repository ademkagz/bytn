...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bytncoin address (e.g. %1)").arg("B7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Bytncoin: URI
...
{
    // Convert bytncoin:// to bytncoin:
    //
    //    Cannot handle this later, because bytncoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bytncoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bytncoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bytncoin.lnk
...
{
    return GetAutostartDir() / "bytncoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bytncoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bytncoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bytncoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bytncoin app to startup item list
