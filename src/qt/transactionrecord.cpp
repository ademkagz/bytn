...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBYTNModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbytnTracker->HasMintTx(hash))) {
            //zBYTN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBYTN;
...
bool IsZBYTNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBYTN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBYTN || type == TransactionRecord::MNReward) {
